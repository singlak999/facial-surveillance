...
//----- labels ----------------------------
        ...
        // Declare a sqlite3 statement pointer
        sqlite3_stmt *stmt;
        int curr_time = time(NULL);
        const char *sql = "SELECT D_Time FROM security WHERE Name = ?;";
        ...
        // Bind the Str value to the parameter
        ...
        // Fetch the result and convert it to a time structure

        const char *result = (const char *)sqlite3_column_text(stmt, 0);
        struct tm *tm = strptime(result, "%H:%M:%S", NULL);

        // Convert the time structure to seconds
        int db_time = tm->tm_hour * 3600 + tm->tm_min * 60 + tm->tm_sec;
        // Finalize the statement
        sqlite3_finalize(stmt);
        // Compare the current time and the database time and check if the difference is greater than 60 seconds
        if (curr_time - db_time > 60)
        { // Prepare a SQL statement to insert data into the security table
            sql = "INSERT INTO security (Name, D_Date, D_Time) VALUES (?, ?, ?);";
            ...
            // Bind the Str value to the first parameter
            rc = sqlite3_bind_text(stmt, 1, Str.c_str(), -1, NULL);
            if (rc != SQLITE_OK)
            { // Print an error message and exit
                std::cerr << "Error binding parameter: " << sqlite3_errmsg(db) << std::endl;
            ...
            // Bind the current date and time to the second and third parameters
            rc = sqlite3_bind_text(stmt, 2, DATE, -1, NULL);
            if (rc != SQLITE_OK)
            { // Print an error message and exit
                std::cerr << "Error binding parameter: " << sqlite3_errmsg(db) << std::endl;
                sqlite3_finalize(stmt);
            ...
            rc = sqlite3_bind_text(stmt, 3, ETIME, -1, NULL);
            if (rc != SQLITE_OK)
            { // Print an error message and exit
                std::cerr << "Error binding parameter: " << sqlite3_errmsg(db) << std::endl;
                sqlite3_finalize(stmt);
